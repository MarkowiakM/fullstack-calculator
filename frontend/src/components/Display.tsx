...
export function Display({ state }: DisplayProps) {
  ...
  return (
    <div className="from-calc-display-start to-calc-display-end min-h-[132px] rounded-[22px] bg-gradient-to-b p-6 pb-5">
      <div className="flex min-h-[20px] items-center justify-between gap-3">
        ...
        {isError && (
          <span className="text-calc-error border-calc-error/45 bg-calc-error/15 font-dm-mono shrink-0 rounded-full border px-2.5 py-1 text-[11px] whitespace-nowrap">
            {state.code}
          </span>
        )}
      </div>
      <div
        ...
        aria-live="polite"
        aria-atomic="true"
        className={`mt-2.5 text-right leading-tight font-semibold break-words tabular-nums ${
          isError ? "text-calc-error" : "text-calc-text"
        }`}
        style={{ fontSize: resultFontSizePx(state) }}
      >
        {value}
      </div>
    </div>
